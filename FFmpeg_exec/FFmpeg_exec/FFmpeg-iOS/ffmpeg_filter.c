    ...
    if (codec && codec->sample_fmts) {
        ...
        if (*p == -1) {
            if((codec->capabilities & AV_CODEC_CAP_LOSSLESS) && av_get_sample_fmt_name(st->codecpar->format) > av_get_sample_fmt_name(codec->sample_fmts[0]))
                GTV_ERROR( "Conversion will not be lossless.\n");
...
static void init_input_filter(FilterGraph *fg, AVFilterInOut *in)
{
    ...
    if (type != AVMEDIA_TYPE_VIDEO && type != AVMEDIA_TYPE_AUDIO) {
        GTV_ERROR( "Only video and audio filters supported "
               "currently.\n");
        exit_program(1);
    }
    ...
    if (in->name) {
        ...
        if (file_idx < 0 || file_idx >= nb_input_files) {
            GTV_ERROR( "Invalid file index %d in filtergraph description %s.\n",
        ...
        if (!st) {
            GTV_ERROR( "Stream specifier '%s' in filtergraph description %s "
                   "matches no streams.\n", p, fg->graph_desc);
            exit_program(1);
        }
        ...
        if (i == nb_input_streams) {
            GTV_ERROR( "Cannot find a matching stream for "
                       ...
                       const char *filter_name)
{
    ...
    if (!trim) {
        GTV_ERROR( "%s filter not present, cannot limit "
               "recording time.\n", name);
    ...
    if (ret < 0) {
        GTV_ERROR( "Error configuring the %s filter", name);
        return ret;
    }
...
#define AUTO_INSERT_FILTER(opt_name, filter_name, arg) do {                 \
    AVFilterContext *filt_ctx;                                              \
                                                                            \
    GTV_ERROR( opt_name " is forwarded to lavfi "            \
...
{
    if (!ofilter->ost) {
        GTV_ERROR( "Filter %s has an unconnected output\n", ofilter->name);
        exit_program(1);
    }
...
static int sub2video_prepare(InputStream *ist, InputFilter *ifilter)
{
    ...
    if (!(w && h)) {
       ...
       GTV_ERROR( "sub2video: using %dx%d canvas\n", w, h);
    }
                                        ...
                                        AVFilterInOut *in)
{
    ...
    if (ist->dec_ctx->codec_type == AVMEDIA_TYPE_AUDIO) {
        GTV_ERROR( "Cannot connect video filter to audio input\n");
        ret = AVERROR(EINVAL);
        goto fail;
    }
                                        ...
                                        AVFilterInOut *in)
{
    ...
    if (ist->dec_ctx->codec_type != AVMEDIA_TYPE_AUDIO) {
        GTV_ERROR( "Cannot connect audio filter to non audio input\n");
...
//     }
    ...
    if (audio_volume != 256) {
        ...
        GTV_ERROR( "-vol has been deprecated. Use the volume "
...
int configure_filtergraph(FilterGraph *fg)
{
    ...
    if (simple) {
		...
		//GTV_ERROR( "Encoder xxxxxxxxxxxxxxxxxxxxx e->value:%d\n",e->value);
    } else {
        fg->graph->nb_threads = filter_complex_nbthreads;
		//GTV_ERROR( "Encoder xxxxxxxxxxxxxxxxxxxxx2 fg->graph->nb_threads :%d\n",fg->graph->nb_threads );
    }
    ...
    if (simple && (!inputs || inputs->next || !outputs || outputs->next)) {
        ...
        GTV_ERROR( "Simple filtergraph '%s' was expected "
    ...
    for (i = 0; i < fg->nb_outputs; i++) {
        ...
        if (!ost->enc) {
            ...
            GTV_ERROR( "Encoder (codec %s) not found for output stream #%d:%d\n",

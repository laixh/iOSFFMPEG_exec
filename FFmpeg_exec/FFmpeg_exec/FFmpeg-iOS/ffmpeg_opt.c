...
int filter_nbthreads = 8;
int filter_complex_nbthreads = 8;
...
{
    GTV_ERROR( "Option '%s' was removed. "
...
{
    GTV_ERROR( "This option is deprecated, use -channel.\n");
    return opt_default(optctx, "channel", arg);
}

static int opt_video_standard(void *optctx, const char *opt, const char *arg)
{
    GTV_ERROR( "This option is deprecated, use -standard.\n");
...
static int opt_map(void *optctx, const char *opt, const char *arg)
{
    ...
    if (sync = strchr(map, ',')) {
        *sync = 0;
        ...
        if (sync_file_idx >= nb_input_files || sync_file_idx < 0) {
            GTV_ERROR( "Invalid sync file index: %d.\n", sync_file_idx);
            exit_program(1);
        }
        if (*sync)
            sync++;
        ...
        if (i == input_files[sync_file_idx]->nb_streams) {
            GTV_ERROR( "Sync stream specification in map %s does not "
                                       "match any streams.\n", arg);
            exit_program(1);
        }
    }
    ...
    if (map[0] == '[') {
        ...
        if (!m->linklabel) {
            GTV_ERROR( "Invalid output link label: %s.\n", map);
            exit_program(1);
        }
    } else {
        ...
        if (file_idx >= nb_input_files || file_idx < 0) {
            GTV_ERROR( "Invalid input file index: %d.\n", file_idx);
            exit_program(1);
        }
        if (negative)
    ...
    if (!m) {
        if (allow_unused) {
            GTV_ERROR( "Stream map '%s' matches no streams; ignoring.\n", arg);
        } else {
            GTV_ERROR( "Stream map '%s' matches no streams.\n"
...
static int opt_map_channel(void *optctx, const char *opt, const char *arg)
{
    ...
    if (n != 3 && n != 5) {
        GTV_ERROR( "Syntax error, mapchan usage: "
    ...
    if (m->file_idx < 0 || m->file_idx >= nb_input_files) {
        GTV_ERROR( "mapchan: invalid input file index: %d\n",
               m->file_idx);
        exit_program(1);
    }
    if (m->stream_idx < 0 ||
        m->stream_idx >= input_files[m->file_idx]->nb_streams) {
        GTV_ERROR( "mapchan: invalid input file stream index #%d.%d\n",
               m->file_idx, m->stream_idx);
        exit_program(1);
    }
    st = input_files[m->file_idx]->ctx->streams[m->stream_idx];
    if (st->codecpar->codec_type != AVMEDIA_TYPE_AUDIO) {
        GTV_ERROR( "mapchan: stream #%d.%d is not an audio stream.\n",
               m->file_idx, m->stream_idx);
        exit_program(1);
    }
    ...
    if (m->channel_idx < 0 || m->channel_idx >= st->codecpar->channels) {
        if (allow_unused) {
            GTV_ERROR( "mapchan: invalid audio channel #%d.%d.%d\n",
                    m->file_idx, m->stream_idx, m->channel_idx);
        } else {
            GTV_ERROR(  "mapchan: invalid audio channel #%d.%d.%d\n"
...
{
    if (filter_hw_device) {
        GTV_ERROR( "Only one filter device can be used.\n");
        return AVERROR(EINVAL);
    }
    filter_hw_device = hw_device_get_by_name(arg);
    if (!filter_hw_device) {
        GTV_ERROR( "Invalid filter device %s.\n", arg);
        return AVERROR(EINVAL);
    }
    return 0;
}
    ...
    if (*arg) {
        *type = *arg;
        ...
        case 'p':
            ...
            break;
        default:
            GTV_ERROR( "Invalid metadata type %c.\n", *arg);
            exit_program(1);
        }
    } else
        *type = 'g';
...
#define METADATA_CHECK_INDEX(index, nb_elems, desc)\
    if ((index) < 0 || (index) >= (nb_elems)) {\
        GTV_ERROR( "Invalid %s index %d while processing metadata maps.\n",\
                (desc), (index));\
        exit_program(1);\
    }
...
#define SET_DICT(type, meta, context, index)\
    ...
    if (type_in == 's') {
        ...
        if (!meta_in) {
            GTV_ERROR( "Stream specifier %s does not match  any streams.\n", istream_spec);
            exit_program(1);
        }
    }
...
static int opt_recording_timestamp(void *optctx, const char *opt, const char *arg)
{
    ...
    GTV_ERROR( "%s is deprecated, set the 'creation_time' metadata "
                                 "tag instead.\n", opt);
    return 0;
}
...
static AVCodec *find_codec_or_die(const char *name, enum AVMediaType type, int encoder)
{
    ...
    if (!codec && (desc = avcodec_descriptor_get_by_name(name))) {
        ...
        if (codec)
            GTV_ERROR( "Matched %s '%s' for codec '%s'.\n",
                   codec_string, codec->name, desc->name);
    }

    if (!codec) {
        GTV_ERROR( "Unknown %s '%s'\n", codec_string, name);
        exit_program(1);
    }
    if (codec->type != type) {
        GTV_ERROR( "Invalid %s type '%s'\n", codec_string, name);
        exit_program(1);
    }
    return codec;
}
...
static void add_input_streams(OptionsContext *o, AVFormatContext *ic)
{
    ...
    for (i = 0; i < ic->nb_streams; i++) {
        ...
        if (discard_str && av_opt_eval_int(&cc, discard_opt, discard_str, &ist->user_set_discard) < 0) {
            GTV_ERROR( "Error parsing discard %s.\n",
                    discard_str);
            exit_program(1);
        }
        ...
        if (!ist->dec_ctx) {
            GTV_ERROR( "Error allocating the decoder context.\n");
            exit_program(1);
        }

        ret = avcodec_parameters_to_context(ist->dec_ctx, par);
        if (ret < 0) {
            GTV_ERROR( "Error initializing the decoder context.\n");
            exit_program(1);
        }
        ...
        case AVMEDIA_TYPE_VIDEO:
            if(!ist->dec)
                //laixhcode
                //ist->dec= avcodec_find_decoder_by_name("h264_videotoolbox");
            ...
            if (framerate && av_parse_video_rate(&ist->framerate,
                                                 framerate) < 0) {
                GTV_ERROR( "Error parsing framerate %s.\n",
                       framerate);
                exit_program(1);
            }
            ...
            if (hwaccel) {
                if (!strcmp(hwaccel, "none"))
                    ...
                    if (!ist->hwaccel_id) {
                        GTV_ERROR( "Unrecognized hwaccel: %s.\n",
                               hwaccel);
                        GTV_ERROR( "Supported hwaccels: ");
                        for (i = 0; hwaccels[i].name; i++)
                            GTV_ERROR( "%s ", hwaccels[i].name);
                        GTV_ERROR( "\n");
                        exit_program(1);
                    }
                }
            ...
            if (hwaccel_output_format) {
                ...
                if (ist->hwaccel_output_format == AV_PIX_FMT_NONE) {
                    GTV_ERROR( "Unrecognised hwaccel output "
        ...
        case AVMEDIA_TYPE_SUBTITLE: {
            ...
            if (canvas_size &&
                ...
                GTV_ERROR( "Invalid canvas size: %s.\n", canvas_size);
                exit_program(1);
            }
            break;
        }
    ...
    if (!file_overwrite) {
        ...
        if (proto_name && !strcmp(proto_name, "file") && avio_check(filename, 0) == 0) {
            if (stdin_interaction && !no_file_overwrite) {
                ...
                if (!read_yesno()) {
                    GTV_ERROR( "Not overwriting - exiting\n");
                    exit_program(1);
                }
                term_init();
            }
            else {
                GTV_ERROR( "File '%s' already exists. Exiting.\n", filename);
                exit_program(1);
            }
        }
    }
...
static void dump_attachment(AVStream *st, const char *filename)
{
    ...
    if (!st->codecpar->extradata_size) {
        GTV_ERROR( "No extradata to dump in stream #%d:%d.\n",
    ...
    if (!*filename && (e = av_dict_get(st->metadata, "filename", NULL, 0)))
        filename = e->value;
    if (!*filename) {
        GTV_ERROR( "No filename specified and no 'filename' tag"
               "in stream #%d:%d.\n", nb_input_files - 1, st->index);
        exit_program(1);
    }
    ...
    if ((ret = avio_open2(&out, filename, AVIO_FLAG_WRITE, &int_cb, NULL)) < 0) {
        GTV_ERROR( "Could not open file %s for writing.\n",
               filename);
        exit_program(1);
    }
...
static uint8_t *get_line(AVIOContext *s)
{
    ...
    if (avio_open_dyn_buf(&line) < 0) {
        GTV_ERROR( "Could not alloc buffer for reading preset.\n");
        exit_program(1);
    }
...
static int choose_encoder(OptionsContext *o, AVFormatContext *s, OutputStream *ost)
{
    ...
    if (type == AVMEDIA_TYPE_VIDEO || type == AVMEDIA_TYPE_AUDIO || type == AVMEDIA_TYPE_SUBTITLE) {
        ...
        if (!codec_name) {
            ...
            if (!ost->enc) {
                GTV_ERROR( "Automatic encoder selection failed for "
...
static OutputStream *new_output_stream(OptionsContext *o, AVFormatContext *oc, enum AVMediaType type, int source_index)
{
    ...
    if (!st) {
        GTV_ERROR( "Could not alloc stream.\n");
        exit_program(1);
    }
    ...
    if (ret < 0) {
        GTV_ERROR( "Error selecting an encoder for stream "
    ...
    if (!ost->enc_ctx) {
        GTV_ERROR( "Error allocating the encoding context.\n");
        exit_program(1);
    }
    ...
    if (!ost->ref_par) {
        GTV_ERROR( "Error allocating the encoding parameters.\n");
        exit_program(1);
    }
    ...
    if (ost->enc) {
        ...
        if (preset && (!(ret = get_preset_file_2(preset, ost->enc->name, &s)))) {
            do  {
                ...
                if (!(arg = strchr(buf, '='))) {
                    GTV_ERROR( "Invalid line found in the preset file.\n");
                    exit_program(1);
                }
                *arg++ = 0;
    ...
    for (i = 0; i<o->nb_max_frames; i++) {
        ...
        if (!*p && type != AVMEDIA_TYPE_VIDEO) {
            GTV_ERROR( "Applying unspecific -frames to non video streams, maybe you meant -vframes ?\n");
            break;
        }
    }
    ...
    while (bsfs && *bsfs) {
        ...
        if (!filter) {
            GTV_ERROR( "Unknown bitstream filter %s\n", bsf_name);
            exit_program(1);
        }
        ...
        if (ret < 0) {
            GTV_ERROR( "Error allocating a bitstream filter context\n");
            exit_program(1);
        }
        ...
        if (bsf_options_str && filter->priv_class) {
            ...
            if (ret < 0) {
                GTV_ERROR( "Error parsing options for bitstream filter %s\n", bsf_name);
                exit_program(1);
            }
        }
        av_freep(&bsf);
...
static void parse_matrix_coeffs(uint16_t *dest, const char *str)
{
    ...
    for (i = 0;; i++) {
        ...
        p = strchr(p, ',');
        if (!p) {
            GTV_ERROR( "Syntax error in matrix \"%s\" at coeff %d\n", str, i);
            exit_program(1);
        }
        p++;
    }
...
static uint8_t *read_file(const char *filename)
{
    ...
    if (ret < 0) {
        GTV_ERROR( "Error opening file %s.\n", filename);
        return NULL;
    }
                             ...
                             OutputStream *ost)
{
    ...
    if (ost->filters_script && ost->filters) {
        GTV_ERROR( "Both -filter and -filter_script set for "
...
static OutputStream *new_video_stream(OptionsContext *o, AVFormatContext *oc, int source_index)
{
    ...
    if (frame_rate && av_parse_video_rate(&ost->frame_rate, frame_rate) < 0) {
        GTV_ERROR( "Invalid framerate value: %s\n", frame_rate);
        exit_program(1);
    }
    if (frame_rate && video_sync_method == VSYNC_PASSTHROUGH)
        GTV_ERROR( "Using -vsync 0 and -r can produce invalid output files\n");
    ...
    if (frame_aspect_ratio) {
        ...
        if (av_parse_ratio(&q, frame_aspect_ratio, 255, 0, NULL) < 0 ||
            q.num <= 0 || q.den <= 0) {
            GTV_ERROR( "Invalid aspect ratio: %s\n", frame_aspect_ratio);
            exit_program(1);
        }
    ...
    if (!ost->stream_copy) {
        ...
        if (frame_size && av_parse_video_size(&video_enc->width, &video_enc->height, frame_size) < 0) {
            GTV_ERROR( "Invalid frame size: %s.\n", frame_size);
            exit_program(1);
        }
        ...
        if (frame_pix_fmt && (video_enc->pix_fmt = av_get_pix_fmt(frame_pix_fmt)) == AV_PIX_FMT_NONE) {
            GTV_ERROR( "Unknown pixel format requested: %s.\n", frame_pix_fmt);
            exit_program(1);
        }
        ...
        if (intra_matrix) {
            if (!(video_enc->intra_matrix = av_mallocz(sizeof(*video_enc->intra_matrix) * 64))) {
                GTV_ERROR( "Could not allocate memory for intra matrix.\n");
                exit_program(1);
            }
        ...
        if (inter_matrix) {
            if (!(video_enc->inter_matrix = av_mallocz(sizeof(*video_enc->inter_matrix) * 64))) {
                GTV_ERROR( "Could not allocate memory for inter matrix.\n");
                exit_program(1);
            }
        ...
        for (i = 0; p; i++) {
            ...
            if (e != 3) {
                GTV_ERROR( "error parsing rc_override\n");
                exit_program(1);
            }
            ...
            if (!video_enc->rc_override) {
                GTV_ERROR( "Could not (re)allocate memory for rc_override.\n");
                exit_program(1);
            }
        ...
        if (do_pass) {
            ...
            if (!strcmp(ost->enc->name, "libx264")) {
                ...
                if (video_enc->flags & AV_CODEC_FLAG_PASS2) {
                    ...
                    if (!logbuffer) {
                        GTV_ERROR( "Error reading log file '%s' for pass-2 encoding\n",
                               logfilename);
                        exit_program(1);
                    }
...
static OutputStream *new_audio_stream(OptionsContext *o, AVFormatContext *oc, int source_index)
{
    ...
    if (!ost->stream_copy) {
        ...
        if (sample_fmt &&
            ...
            GTV_ERROR( "Invalid sample format '%s'\n", sample_fmt);
            exit_program(1);
        }
        ...
        for (n = 0; n < o->nb_audio_channel_maps; n++) {
            ...
            if ((map->ofile_idx   == -1 || ost->file_index == map->ofile_idx) &&
                ...
                if (map->channel_idx == -1) {
                    ...
                    GTV_ERROR( "Cannot determine input stream for channel mapping %d.%d\n",
...
static OutputStream *new_data_stream(OptionsContext *o, AVFormatContext *oc, int source_index)
{
    ...
    if (!ost->stream_copy) {
        GTV_ERROR( "Data stream encoding not supported yet (only streamcopy)\n");
        exit_program(1);
    }
...
static OutputStream *new_unknown_stream(OptionsContext *o, AVFormatContext *oc, int source_index)
{
    ...
    if (!ost->stream_copy) {
        GTV_ERROR( "Unknown stream encoding not supported yet (only streamcopy)\n");
        exit_program(1);
    }
...
static int read_ffserver_streams(OptionsContext *o, AVFormatContext *s, const char *filename)
{
    ...
    for(i=0;i<ic->nb_streams;i++) {
        ...
        if (!codec) {
            GTV_ERROR( "no encoder found for codec id %i\n", ic->streams[i]->codecpar->codec_id);
            return AVERROR(EINVAL);
        }
                               ...
                               AVFormatContext *oc)
{
    ...
    default:
        GTV_ERROR( "Only video and audio filters are supported "
               "currently.\n");
        exit_program(1);
    }
    ...
    if (ost->stream_copy) {
        GTV_ERROR( "Streamcopy requested for output stream %d:%d, "
...
static int opt_target(void *optctx, const char *opt, const char *arg)
{
    ...
    if (!strncmp(arg, "pal-", 4)) {
        ...
        if (norm != UNKNOWN)
            GTV_ERROR( "Assuming %s for target.\n", norm == PAL ? "PAL" : "NTSC");
    }

    if (norm == UNKNOWN) {
        GTV_ERROR( "Could not determine norm (PAL/NTSC/NTSC-Film) for target.\n");
        GTV_ERROR( "Please prefix target with \"pal-\", \"ntsc-\" or \"film-\",\n");
        GTV_ERROR( "or set a framerate with \"-r xxx\".\n");
        exit_program(1);
    }
...
static int opt_vstats(void *optctx, const char *opt, const char *arg)
{
    ...
    if (!today) { // maybe tomorrow
        GTV_ERROR( "Unable to get current time: %s\n", strerror(errno));
        exit_program(1);
    }
...
static int opt_preset(void *optctx, const char *opt, const char *arg)
{
    ...
    if (!(f = get_preset_file(filename, sizeof(filename), arg, *opt == 'f', codec_name))) {
        if(!strncmp(arg, "libx264-lossless", strlen("libx264-lossless"))){
            GTV_ERROR( "Please use -preset <speed> -qp 0\n");
        }else
            GTV_ERROR( "File for preset '%s' not found\n", arg);
        exit_program(1);
    }
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        if (!av_strtok(key,   "=",    &value) ||
            !av_strtok(value, "\r\n", &endptr)) {
            GTV_ERROR( "%s: Invalid syntax: '%s'\n", filename, line);
            exit_program(1);
        }
       GTV_ERROR( "ffpreset[%s]: set '%s' = '%s'\n", filename, key, value);
        ...
        if      (!strcmp(key, "acodec")) opt_audio_codec   (o, key, value);
            ...
            GTV_ERROR( "%s: Invalid option or argument: '%s', parsed as '%s' = '%s'\n",
...
static int opt_bitrate(void *optctx, const char *opt, const char *arg)
{
    ...
    if(!strcmp(opt, "ab")){
        ...
        GTV_ERROR( "Please use -b:a or -b:v, -b is ambiguous\n");
...
static int opt_qscale(void *optctx, const char *opt, const char *arg)
{
    ...
    if(!strcmp(opt, "qscale")){
        GTV_ERROR( "Please use -q:a or -q:v, -qscale is ambiguous\n");
...
static int opt_profile(void *optctx, const char *opt, const char *arg)
{
    ...
    if(!strcmp(opt, "profile")){
        GTV_ERROR( "Please use -profile:a or -profile:v, -profile is ambiguous\n");
...
static int opt_channel_layout(void *optctx, const char *opt, const char *arg)
{
    ...
    if (!layout) {
        GTV_ERROR( "Unknown channel layout: %s\n", arg);
        return AVERROR(EINVAL);
    }
...
void show_help_default(const char *opt, const char *arg)
{
    ...
    if (opt && *opt) {
        if (!strcmp(opt, "long"))
            ...
            GTV_ERROR( "Unknown help option '%s'.\n", opt);
    }
...
{
    GTV_ERROR( "Hyper fast Audio and Video encoder\n");
    GTV_ERROR( "usage: %s [options] [[infile options] -i infile]... {[outfile options] outfile}...\n", program_name);
    GTV_ERROR( "\n");
}
...
int ffmpeg_parse_options(int argc, char **argv)
{
    ...
    if (ret < 0) {
        GTV_ERROR( "Error splitting the argument list: ");
        goto fail;
    }
    ...
    if (ret < 0) {
        GTV_ERROR( "Error parsing global options: ");
        goto fail;
    }
...
static int opt_progress(void *optctx, const char *opt, const char *arg)
{
    ...
    if (ret < 0) {
        GTV_ERROR( "Failed to open progress URL \"%s\": %s\n",
               arg, av_err2str(ret));
        return ret;
    }
    progress_avio = avio;
